...
do
	echo "Getting session data for sessions numbered N = $M0 to $N0 with name format ${name_prefix}-N:" 1>&2
	...
	do
		...
		if [[ -z $sessions[$npn] ]]
		then
			echo "WARN: Session $npn wasn't found in 'anyscale list sessions'. Skipping..." 1>&2
		else
			if [[ -z $NOOP ]]
			then
